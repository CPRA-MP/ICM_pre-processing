...
observed_df = pd.read_csv(observed_file, parse_dates=["yyyymmdd"])
...
observed_df["year"] = observed_df["yyyymmdd"].dt.year
observed_df["month"] = observed_df["yyyymmdd"].dt.month
...
while start <= end:
    ...
    days_in_month = historical_monthly_data["yyyymmdd"].dt.day.values  # array of month days
    ...
    historical_monthly_data["yyyymmdd"] = [datetime(year, month, day) for day in days_in_month]
...
future_timeseries_df['yyyymmdd'] = pd.to_datetime(future_timeseries_df['yyyymmdd'])
future_timeseries_df = future_timeseries_df.set_index('yyyymmdd')
...
future_final_df.to_csv(output_filename, index=True, index_label="yyyymmdd")
